PROGRAM blend_precip_downscale_gammanoise
...
REAL, ALLOCATABLE, DIMENSION(:,:) :: climo_prob ! climatological prob of analyses
...
CALL read_cdf_netcdf(nxa,nya,npct,nens_cmc,ipcpvar,thresh,precip_anal_cdf,&
     ...
     rlatsa, rlonsa)
PRINT *,'lon,lat(479,199) = ', rlonsa(479,199), rlatsa(479,199)

PRINT *, 'after read_cdf_netcdf, precip_anal_cdf(479,199,:) = ', precip_anal_cdf(479,199,:)
...
IF (ipcpvar .eq. 0) THEN ! probabilistic forecast
    ...
    PRINT *,'dressing file = ',dressing_infile
    CALL read_dressing_parameters (n_amounts, dressing_infile, ramt, &
    ...
    WRITE (6,*) 'Calling ensemble_probs_x9_weighted'
    ...
    prob_forecast_smoothed = prob_forecast_cdf  ! prob_forecast_cdf is raw/jittered MME outside of conusmask
    WRITE (6,*)'Calling sgolay_smooth_x9'

    PRINT *,'Driver: conusmask(42:82,93) = ',conusmask(42:82,93)
    CALL sgolay_smooth_x9(nxa, nya, ipcpvar, prob_forecast_smoothed, prob_forecast_cdf, &
        conusmask, weights, window_size, order, istat)
    prob_forecast = raw_weight*prob_forecast_cdf + (1.-raw_weight)*prob_forecast_smoothed
...
WRITE (6,fmt='(A10,1X,3(F10.5,1X))')'CMC',minval(cmc_control_ccpa),&
    maxval(cmc_control_ccpa),sum(cmc_control_ccpa)/(nxa*nya*nens_cmc)
WRITE (6,fmt='(A10,1X,3(F10.5,1X)/)')'CMC ENS',&
    minval(cmc_ensemble_ccpa),maxval(cmc_ensemble_ccpa),&
    sum(cmc_ensemble_ccpa)/(nxa*nya)
...
DEALLOCATE(rlonsa, rlatsa, precip_anal_fine, prob_forecast, prob_forecast_raw, &
    ...
    ncep_ensemble_ccpa, topo_eighth, prob_forecast_raw_ecmwf, &
    ...
    ramt, closest_histogram, prob_forecast_smoothed, stat=ios)

WRITE (6,*)'Deallocation Status = ',ios
WRITE (6,*)'Done!'

END PROGRAM blend_precip_downscale_gammanoise
