SUBROUTINE sgolay_smooth_x9(nx, ny, ideterministic, z, zraw, &
     ihavedata, weights, window_size, order, istat)
...
!  
! additional code put in May 2016 to try to add more graceful transition to raw ensemble
!    at boundaries of the conusmask.  See bottom of routine.

INTEGER, INTENT(IN) :: nx ! row dimension of z
INTEGER, INTENT(IN) :: ny ! column dimension of z
INTEGER, INTENT(IN) :: ideterministic ! = 1 if deterministic forecast data, 0 if probability
INTEGER, INTENT(IN) :: window_size ! as defined in Press et al. Numerical Recipes
INTEGER, INTENT(IN) :: order
INTEGER*2, INTENT(IN), DIMENSION(nx,ny) :: ihavedata
REAL*4, INTENT(INOUT), DIMENSION(nx,ny) :: z
REAL*4, INTENT(INOUT), DIMENSION(nx,ny) :: zraw  ! raw MME data
...
CALL barnes_like(ihavedata, z, nx, ny, ideterministic, z2)
z = z2
...
ELSE
   ...
   ! ---- perform a convolution via FFT.  *** NOTE! I haven't been able to get this to work properly.
   !      The FFT method should be much, much faster, but the damn thing doesn't work yet.
...
! ---- last step, added May 2016: at edges of CONUS, transition from S-G smoothed values to 
!      raw input values, and make sure if the input data is probability data, the 
!      results are bounded by 0 and 1.

zsaved = z
DO i = 1, nx
   DO j = 1, ny

      IF (zsaved(i,j) .gt. 1.0 .and. ideterministic .eq. 0) zsaved(i,j) = 1.0
      IF (ihavedata(i,j) .ne. 1) THEN

         ! ---- now more complicated procedure introduced here, a linear combination of
         !      the smoothed forecast produced by Savitky-Golay and raw, the weight
         !      depending on how far one is from the nearest point with good data.
         !      for each point outside conus, determine the closest distance to a point
         !      inside the conus, and use this to set a distance dependent weighting.
         !      Locations near the CONUS edge use more of the S-G smoothed value, less
         !      of the raw ensemble
         ...
         DO jsub = jmin,jmax
            DO isub = imin,imax
               IF (ihavedata(isub,jsub) .eq. 1) THEN
                  dist2 = (i-isub)**2 + (j-jsub)**2
         ...
         IF (dmin .lt. 99998.) THEN
            weight_smoothed = (256. - dmin) / (dmin + 256.)
            IF (weight_smoothed .lt. 0) weight_smoothed = 0.
            IF (weight_smoothed .gt. 1.) weight_smoothed = 1.
         ELSE
            weight_smoothed = 0.
         ENDIF
         z(i,j) = weight_smoothed*zsaved(i,j) + (1.-weight_smoothed)*zraw(i,j)
     ELSE
         z(i,j) = zsaved(i,j)
         IF (z(i,j) .lt. 0.0) z(i,j) = 0.0
         IF (z(i,j) .gt. 1.0 .and. ideterministic .eq. 0) z(i,j) = 1.0
      ENDIF
   END DO
END DO

RETURN
END SUBROUTINE sgolay_smooth_x9
