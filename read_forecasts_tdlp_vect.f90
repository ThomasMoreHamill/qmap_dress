subroutine read_forecasts_tdlp_vect(nxf,nyf,nens_cmc,nens_ncep,ipcpvar,idate,ifhr,cmc_ctl,ncep_ctl,&
...
write(6,100)idate   
100 format(/'Reading 1/8 deg. Vector TDLPACK model data for date = ',I10) 

! ---- Get CMC Deterministic (Unit = 60), Model DD = 63.
kunit=60
if(ifirst.eq.0)open(unit=kunit,form='unformatted',status='old',convert='big_endian')
dd=63
id(1)=id1+dd
call tdlp_get_vect(kunit,id,idate,nxf,nyf,xdata,ier)
if(ier.eq.-1)then
   close(kunit)
...
! ---- Get CMC Ensemble (Unit = 61); 20 members, Model DD = 41-60
kunit=61
if(ifirst.eq.0)open(unit=kunit,form='unformatted',status='old',convert='big_endian')
do j=1,nens_cmc
   dd=j+40
...
! ---- Get ECMWF Deterministic (Unit = 62)
! ---- Get ECMWF Ensemble (Unit = 63)

! ---- Get NCEP Control (i.e. GFS) (Unit = 64), Model DD = 08
kunit=64
if(ifirst.eq.0)open(unit=kunit,form='unformatted',status='old',convert='big_endian')
dd=08
id(1)=id1+dd
...
! ---- Get NCEP Ensemble (i.e. GEFS) (Unit = 65); 20 members, Model DD = 11-30
kunit=65
if(ifirst.eq.0)open(unit=kunit,form='unformatted',status='old',convert='big_endian')
do j=1,nens_ncep
   dd=j+10
...
! ---- Get UKMet Deterministic (Unit = 66)
! ---- Get UKMet Ensemble (Unit = 67)

ifirst=ifirst+1
return
end subroutine read_forecasts_tdlp_vect
