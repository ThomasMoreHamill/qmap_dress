SUBROUTINE raw_ensemble_probs_x9_local(nxa, nya, nens_cmc, nens_ncep, nens_ecmwf, &
...
REAL, ALLOCATABLE, DIMENSION(:,:,:) :: raw_ensemble_ecmwf
...
ALLOCATE (raw_ensemble(nxa,nya,nens*9),raw_ensemble_ncep(nxa,nya,nens_ncep*9), &
   raw_ensemble_cmc(nxa,nya,nens_cmc*9), raw_ensemble_ecmwf(nxa,nya,nens_ecmwf*9) )
        ...
        DO jya = 1, nya
            DO ixa = 1, nxa
                DO i9 = 1,9
                    ...
                    raw_ensemble_cmc(ixa,jya,iktr_cmc(ixa,jya)) = ncep_ensemble_ccpa_x9(i9,ixa,jya,imem)
                END DO  ! i9
            END DO     ! ixa
        END DO        ! jya
    ENDIF
        ...
        DO jya = 1, nya
            DO ixa = 1, nxa
                DO i9 = 1,9
                    ...
                    raw_ensemble(ixa,jya,iktr(ixa,jya)) = ecmwf_ensemble_ccpa_x9(i9,ixa,jya,imem)
                    raw_ensemble_cmc(ixa,jya,iktr_cmc(ixa,jya)) = ecmwf_ensemble_ccpa_x9(i9,ixa,jya,imem)
                END DO  ! i9
            END DO     ! ixa
        END DO        ! jya
    ENDIF
    ...
    DO ixa = 1, nxa
        ...
        IF (iktr_ecmwf(ixa,jya) .gt. 0) THEN
            DO imem = 1, iktr_ecmwf(ixa,jya)
                IF (raw_ensemble_ecmwf(ixa,jya,imem) .ge. rthresh) ncount_ecmwf = ncount_ecmwf + 1
            END DO
...
write (6,fmt='(A10,1X,3(F10.5,1X))')'NCEP', minval(prob_forecast_raw_NCEP), &
    maxval(prob_forecast_raw_NCEP), sum(prob_forecast_raw_NCEP)/(nxa*nya)
write (6,fmt='(A10,1X,3(F10.5,1X))')'CMC',minval(prob_forecast_raw_CMC), &
    maxval(prob_forecast_raw_CMC), sum(prob_forecast_raw_CMC)/(nxa*nya)
write (6,fmt='(A10,1X,3(F10.5,1X))')'ECMWF',minval(prob_forecast_raw_ECMWF), &
    maxval(prob_forecast_raw_ECMWF), sum(prob_forecast_raw_ECMWF)/(nxa*nya)
write (6,fmt='(A10,1X,3(F10.5,1X)/)')'COMBINED',minval(prob_forecast_raw), &
    maxval(prob_forecast_raw), sum(prob_forecast_raw)/(nxa*nya)

! ---- Deallocate the arrays

DEALLOCATE(raw_ensemble, raw_ensemble_cmc, raw_ensemble_ncep, raw_ensemble_ecmwf)
